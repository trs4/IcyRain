...
internal static class GrpcProtocolHelpers
{
    public static bool TryDecodeTimeout(StringValues values, out TimeSpan timeout)
    {
        ...
        if (values.Count == 1)
        {
            ...
            {
                var timeoutUnit = timeoutHeader[^1];
    ...
    public static bool IsCorsPreflightRequest(HttpContext httpContext)
        => HttpMethods.IsOptions(httpContext.Request.Method) && httpContext.Request.Headers.ContainsKey(HeaderNames.AccessControlRequestMethod);
    ...
    public static byte[] ParseBinaryHeader(string base64)
    {
        string decodable = (base64.Length % 4) switch // base64 has the required padding
        {
            ...
            _ => throw new FormatException("Invalid base64 header value"), // length%4 == 1 should be illegal
        };
    ...
    internal static bool ShouldSkipHeader(string name)
        => name.StartsWith(':') || GrpcProtocolConstants.FilteredHeaders.Contains(name);
