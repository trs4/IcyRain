...
/// <summary>Reads and decodes base64 encoded bytes from the inner reader</summary>
internal sealed class Base64PipeReader : PipeReader
{
    ...
    private SequencePosition ResolvePosition(SequencePosition base64Position)
    {
        ...
        else
        {
            ...
            if (paddingSequence.PositionOf((byte)'=') is null)
    ...
    public override async ValueTask<ReadResult> ReadAsync(CancellationToken token = default)
    {
        ...
        var innerResult = await _inner.ReadAsync(token);
        ...
        while (innerResult.Buffer.Length - _currentInnerBufferRead < 4)
        {
            ...
            innerResult = await _inner.ReadAsync(token);
        }
