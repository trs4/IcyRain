...
{
    /// <summary>Updates the <see cref="LoadBalancer"/> with state from the <see cref="Resolver"/></summary>
...
/// <summary>Factory for creating new <see cref="LoadBalancer"/> instances. A factory is used when the load balancer config name matches the factory name
/// <para>Note: Experimental API that can change or be removed without any prior notice</para>
/// </summary>
public abstract class LoadBalancerFactory
{
    internal static readonly LoadBalancerFactory[] KnownLoadBalancerFactories =
    [
        new PickFirstBalancerFactory(),
        new RoundRobinBalancerFactory()
    ];

    /// <summary>Gets the load balancer factory name. A factory is used when the load balancer config name matches the factory name</summary>
    public abstract string Name { get; }

    /// <summary>Creates a new <see cref="LoadBalancer"/> with the specified options</summary>
