...
/// <summary>Configuration for a method. The <see cref="Names"/> collection is used to determine which methods this configuration applies to</summary>
public sealed class MethodConfig : ConfigObject
{
    ...
    private readonly ConfigProperty<Values<MethodName, object>, IList<object>> _names =
        new(i => new Values<MethodName, object>(i ?? [], s => s.Inner, s => new MethodName((IDictionary<string, object>)s)), NamePropertyName);

    private readonly ConfigProperty<RetryPolicy, IDictionary<string, object>> _retryPolicy =
        new(i => i is not null ? new RetryPolicy(i) : null, RetryPolicyPropertyName);

    private readonly ConfigProperty<HedgingPolicy, IDictionary<string, object>> _hedgingPolicy =
        new(i => i is not null ? new HedgingPolicy(i) : null, HedgingPolicyPropertyName);

    /// <summary>Initializes a new instance of the <see cref="MethodConfig"/> class</summary>
    public MethodConfig() { }

    internal MethodConfig(IDictionary<string, object> inner) : base(inner) { }

    /// <summary>Gets or sets the retry policy for outgoing calls. A retry policy can't be combined with <see cref="HedgingPolicy"/></summary>
    ...
    /// </para>
    /// </remarks>
    public IList<MethodName> Names => _names.GetValue(this)!;
}
