...
internal static class CommonGrpcProtocolHelpers
{
    ...
    private const int TimerEpsilonMilliseconds = 14;
    ...
    public static bool IsContentType(string contentType, string? s)
    {
        ...
        if (nextChar == '+')
            return true; // Accept any message format. Marshaller could be set to support third-party formats
    ...
    public static string ConvertToRpcExceptionMessage(Exception ex)
        ...
        => ex.InnerException == null ? $"{ex.GetType().Name}: {ex.Message}" : BuildErrorMessage(ex);
