    ...
    where TResponse : class
{
    ...
    public RetryCall(RetryPolicyInfo retryPolicy, GrpcChannel channel, Method<TRequest, TResponse> method, CallOptions options)
        : base(channel, method, options, retryPolicy.MaxAttempts)
    {
        ...
        try
        {
            ...
            while (true)
            {
                ...
                lock (Lock)
                {
                    ...
                    currentCall = _activeCall = HttpClientCallInvoker.CreateGrpcCall(Channel, Method, Options, AttemptCount, forceAsyncHttpResponse: true, CallWrapper);
                ...
                catch (RpcException ex)
                {
                    ...
                    if (dropValue is not null && bool.TryParse(dropValue, out var isDrop) && isDrop)
                    {
                ...
                if (result is null)
                {
                    ...
                    if (retryPushbackMS is not null)
                    {
...
#pragma warning disable CA2000 // Dispose objects before losing scope
                    IGrpcCall<TRequest, TResponse> resolvedCall = (IsDeadlineExceeded() && !(currentCall.CallTask.IsCompletedSuccessfully
                            && currentCall.CallTask.Result.StatusCode == StatusCode.DeadlineExceeded))
...
#pragma warning restore CA2000 // Dispose objects before losing scope
        ...
        {
            if (CommitedCallTask.IsCompletedSuccessfully && CommitedCallTask.Result is GrpcCall<TRequest, TResponse> call)
            {
    ...
    public override async Task ClientStreamWriteAsync(TRequest message, CancellationToken token)
    {
        using var registration = (token.CanBeCanceled && token != Options.CancellationToken)
            ? RegisterRetryCancellationToken(token)
            : default;

        // The retry client stream writer prevents multiple threads from reaching here.
        await DoClientStreamActionAsync(async call =>
        {
            Debug.Assert(call.ClientStreamWriter is not null);

            if (ClientStreamWriteOptions is not null)
                call.ClientStreamWriter.WriteOptions = ClientStreamWriteOptions;

            call.TryRegisterCancellation(token, out var registration);

            try
            {
                await call.WriteClientStreamAsync(WriteNewMessage, message, token).ConfigureAwait(false);
            }
            finally
            {
    ...
    private Task<IGrpcCall<TRequest, TResponse>?> GetActiveCallAsync(IGrpcCall<TRequest, TResponse>? previousCall)
    {
        ...
        lock (Lock)
        {
            ...
            if (_activeCall is not null && previousCall != _activeCall)
