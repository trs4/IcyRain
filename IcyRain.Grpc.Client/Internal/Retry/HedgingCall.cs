            ...
            lock (Lock)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (_delayInterruptTcs is not null)
                        {
...
#pragma warning restore CA2000 // Dispose objects before losing scope
    ...
    {
        Debug.Assert(_hedgingDelayCts is not null);
        Debug.Assert(_delayInterruptTcs is not null);
        ...
        while (true)
        {
            ...
            lock (Lock)
            {
                ...
                if (_pushbackDelay is not null)
                {
                    ...
                    return; // Immediately return for non-fatal status
            }
        }
    }
        ...
        try
        {
            await DoClientStreamActionAsync(async calls =>
            {
                ...
                for (var i = 0; i < calls.Count; i++)
                {
                    ...
                    if (c.TryRegisterCancellation(token, out var registration))
                    {
                        registrations ??= new List<CancellationTokenRegistration>(calls.Count);
                        registrations.Add(registration.Value);
                    }

                    var writeTask = c.WriteClientStreamAsync(WriteNewMessage, message, token);
