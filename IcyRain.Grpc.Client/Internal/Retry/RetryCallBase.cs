    ...
    where TResponse : class
{
    ...
    public bool ResponseFinished => CommitedCallTask.IsCompletedSuccessfully && CommitedCallTask.Result.ResponseFinished;
    ...
    protected RetryCallBase(GrpcChannel channel, Method<TRequest, TResponse> method, CallOptions options, int retryAttempts)
    {
        Channel = channel;
        Method = method;
        Options = options;
        ...
        {
            var timeout = CommonGrpcProtocolHelpers.GetTimerDueTime(deadline - DateTime.UtcNow, Channel.MaxTimerDueTime);
            CancellationTokenSource.CancelAfter(TimeSpan.FromMilliseconds(timeout));
        }

        if (HasClientStream()) // Run continuation synchronously so awaiters execute inside the lock
            NewActiveCallTcs = new TaskCompletionSource<IGrpcCall<TRequest, TResponse>?>(TaskCreationOptions.None);

        MaxRetryAttempts = retryAttempts > Channel.MaxRetryAttempts ? Channel.MaxRetryAttempts.Value : retryAttempts;
    }
    ...
    public void StartClientStreaming() => StartCore(call =>
    {
    ...
    public void StartServerStreaming(TRequest request) => StartCore(
        call => call.StartServerStreamingCore(CreatePushUnaryContent(request, call)));

    public void StartDuplexStreaming() => StartCore(call =>
    {
        var clientStreamWriter = new HttpContentClientStreamWriter<TRequest, TResponse>(call);
        var content = CreatePushStreamContent(call, clientStreamWriter);
    ...
    private HttpContent CreatePushUnaryContent(TRequest request, GrpcCall<TRequest, TResponse> call)
    {
        ...
        Task WriteAsync(TRequest request, Stream stream)
            => WriteNewMessage(call, stream, call.Options, request);
    }

    private PushStreamContent<TRequest, TResponse> CreatePushStreamContent(
        GrpcCall<TRequest, TResponse> call, HttpContentClientStreamWriter<TRequest, TResponse> clientStreamWriter)
        => new PushStreamContent<TRequest, TResponse>(clientStreamWriter, async requestStream =>
        {
            Task writeTask;
    ...
    public abstract Task ClientStreamWriteAsync(TRequest message, CancellationToken token);

    protected CancellationTokenRegistration RegisterRetryCancellationToken(CancellationToken token)
        => token.Register(
            static state =>
            {
    ...
    protected bool IsDeadlineExceeded() => Options.Deadline is not null && Options.Deadline <= DateTime.UtcNow;

    protected static int? GetRetryPushback(HttpResponseMessage? httpResponse)
    {
    ...
    protected static byte[] SerializePayload(GrpcCall<TRequest, TResponse> call, CallOptions callOptions, TRequest request)
    {
    ...
    protected async Task WriteNewMessage(GrpcCall<TRequest, TResponse> call, Stream writeStream, CallOptions callOptions, TRequest message)
    {
        ...
        lock (Lock)
        {
            ...
            {
                messageData = RetryCallBase<TRequest, TResponse>.SerializePayload(call, callOptions, message);
            ...
            else
            {
                ...
                messageData = BufferedMessages[^1];
            }
        }
    ...
    protected bool HasResponseStream()
        => Method.Type == MethodType.ServerStreaming || Method.Type == MethodType.DuplexStreaming;
    ...
    protected void SetNewActiveCallUnsynchronized(IGrpcCall<TRequest, TResponse> call)
    {
        ...
        if (NewActiveCallTcs is not null)
        {
    ...
    Task IGrpcCall<TRequest, TResponse>.WriteClientStreamAsync<TState>(
        Func<GrpcCall<TRequest, TResponse>, Stream, CallOptions, TState, Task> writeFunc, TState state, CancellationToken token)
    ...
    {
        Debug.Assert(NewActiveCallTcs is not null);
    ...
    protected StatusGrpcCall<TRequest, TResponse> CreateStatusCall(Status status)
        => new StatusGrpcCall<TRequest, TResponse>(status, Channel, Method, MessagesRead, _request)
        {
...
#pragma warning restore CA2000 // Dispose objects before losing scope
    ...
    protected bool IsRetryThrottlingActive() => Channel.RetryThrottling?.IsRetryThrottlingActive() ?? false;

    protected void RetryAttemptCallSuccess() => Channel.RetryThrottling?.CallSuccess();

    protected void RetryAttemptCallFailure() => Channel.RetryThrottling?.CallFailure();
