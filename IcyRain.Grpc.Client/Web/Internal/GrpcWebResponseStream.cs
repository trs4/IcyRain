...
internal sealed class GrpcWebResponseStream : Stream
{
    ...
    public override async ValueTask<int> ReadAsync(Memory<byte> data, CancellationToken token = default)
    {
        ...
        if (data.Length == 0)
        {
            ...
            var read = await _inner.ReadAsync(data, token).ConfigureAwait(false);
            Debug.Assert(read == 0);
            return 0;
        }
            ...
            case ResponseState.Header:
                {
                    ...
                    var success = await TryReadDataAsync(_inner, headerBuffer, token).ConfigureAwait(false);
            ...
            case ResponseState.Trailer:
                {
                    ...
                    if (headerBuffer.Length < HeaderLength)
                    {
                        ...
                        var success = await TryReadDataAsync(_inner, newBuffer.AsMemory(headerBuffer.Length), token).ConfigureAwait(false);
                    ...
                    await ReadTrailersAsync(isCompressed, length, data, token).ConfigureAwait(false);
                    return 0;
                }
            default:
    ...
    private async Task ReadTrailersAsync(bool isCompressed, int trailerLength, Memory<byte> data, CancellationToken token)
    {
        if (trailerLength > 0)
        {
            ...
            var success = await TryReadDataAsync(_inner, data, token).ConfigureAwait(false);
        ...
        //    request as completing successfully if the request and response aren't completely consumed
        var count = await _inner.ReadAsync(data, token).ConfigureAwait(false);
    ...
    private static bool IsBitSet(byte b, int pos) => ((b >> pos) & 1) != 0;

    private static async Task<bool> TryReadDataAsync(Stream responseStream, Memory<byte> buffer, CancellationToken token)
    {
        int read;
        var received = 0;

        while ((read = await responseStream.ReadAsync(buffer.Slice(received, buffer.Length - received), token).ConfigureAwait(false)) > 0)
        {
            received += read;
