...
#pragma warning restore CA2213 // Disposable fields should be disposed
    ...
    public override async ValueTask WriteAsync(ReadOnlyMemory<byte> data, CancellationToken token = default)
    {
        ...
        while (data.Length >= 3)
        {
            ...
            await _inner.WriteAsync(_buffer.AsMemory(0, bytesWritten + base64Remainder), token).ConfigureAwait(false);
        ...
        if (localBuffer.Length < _buffer.Length)
            await _inner.WriteAsync(_buffer.AsMemory(0, 4), token).ConfigureAwait(false);
    ...
    {
        await WriteRemainderAsync(token).ConfigureAwait(false);
        await _inner.FlushAsync(token).ConfigureAwait(false);
    }

    internal async Task WriteRemainderAsync(CancellationToken token)
    {
        if (_remainder > 0)
        {
            EnsureSuccess(Base64.EncodeToUtf8InPlace(_buffer, _remainder, out var bytesWritten));

            await _inner.WriteAsync(_buffer.AsMemory(0, bytesWritten), token).ConfigureAwait(false);
            _remainder = 0;
        }
    }
