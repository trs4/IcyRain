...
{
    internal static class LZ4SegmentDecoder
    {
        public static unsafe ArraySegment<byte> Decode(byte[] source)
        {
            if ((source[0] & 0x07) != 0)
        ...
        public static unsafe ArraySegment<byte> Decode(byte[] source, ref int decodedLength)
        {
            if ((source[0] & 0x07) != 0)
                throw new InvalidOperationException("Header is corrupted");
