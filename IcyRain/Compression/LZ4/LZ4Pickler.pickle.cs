...
namespace IcyRain.Compression.LZ4
{
    ...
    /// </summary>
    internal static partial class LZ4Pickler
    {
            ...
            ReadOnlySpan<byte> source, LZ4Level level, Span<byte> buffer)
        {
            ...
            {
                var headerSize = GetUncompressedHeaderSize(version);
                var result = new byte[headerSize + sourceLength];
                var target = result.AsSpan();
                var offset = EncodeUncompressedHeader(target, version);
        ...
        // ReSharper disable once UnusedParameter.Local
        private static int GetUncompressedHeaderSize(int version) =>
            version switch
            {
                0 => sizeof(byte),
                _ => throw UnexpectedVersion(version),
        ...
        // ReSharper disable once UnusedParameter.Local
        private static int EncodeUncompressedHeader(Span<byte> target, int version)
            => version switch
            {
